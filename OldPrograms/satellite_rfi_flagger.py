...
def precompute_target_positions(target, time_steps, observerSSB): 
    return observerSSB.at(time_steps).observe(target).apparent()#returns apparent position - may not be as appropriate for radio as optical
...
def get_observation_data():
    #demo mode or not
    demo_choice = input("Demo? (y/n): ").strip().lower()

    if demo_choice == "y":
        #sse predefined values for demo
        ...
        #normal input flow for non-demo
        print('Enter date information in LST.')
